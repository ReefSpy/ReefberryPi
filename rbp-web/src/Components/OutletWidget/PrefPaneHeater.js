...
class PrefPaneHeater extends Component {
  ...
  handleProbeChange = (event) => {
    ...
    this.setState({
      selectedTempProbeID:
        this.state.tempProbes[event.target.selectedIndex].probeid,
    });
  };
  ...
  componentDidMount() {
    console.log(this.state.tempProbes);

    for (let probe in this.state.tempProbes) {
      console.log(this.state.tempProbes[probe].probeid);
      if (this.state.tempProbes[probe].probeid === this.state.heater_probe) {
        this.setState({ selectedTempProbeIndex: probe });
        this.setState({
          selectedTempProbe: this.state.tempProbes[probe].probename,
        });
        return;
      }
    }
      ...
      <div>
        <div className="form-row">
          ...
          <select
            ...
            value={this.state.selectedTempProbe}
          >
